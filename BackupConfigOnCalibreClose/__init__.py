...
class BackupConfigOnCalibreClose(LibraryClosedPlugin):
    ...
    def config_widget(self):
        ...
        def add_right_aligned_label(layout, text, row):
        ...
        add_right_aligned_label(l, 'Containing folder:', gr)
        l.addWidget(w, gr, 1)
        ...
        l.addWidget(w, gr, 2)
        gr += 1
        self.config_dir_value = QLabel()
        add_right_aligned_label(l, 'Current value:', gr)
        l.addWidget(self.config_dir_value, gr, 1)
        gr += 1

        self.name_pattern_widget = QLineEdit()
        ...
        add_right_aligned_label(l, 'Backup folder name:', gr)
        l.addWidget(self.name_pattern_widget, gr, 1)
        gr += 1
        self.name_pattern_value = QLabel()
        add_right_aligned_label(l, 'Current value:', gr)
        l.addWidget(self.name_pattern_value, gr, 1)
        gr += 1

        self.date_pattern_widget = QLineEdit()
        ...
        add_right_aligned_label(l, 'File name:', gr)
        l.addWidget(self.date_pattern_widget, gr, 1)
        gr += 1
        self.date_pattern_value = QLabel()
        add_right_aligned_label(l, 'Current value:', gr)
        l.addWidget(self.date_pattern_value, gr, 1)
        gr += 1

        self.first_days_widget = QSpinBox()
        self.first_days_widget.setValue(plugin_prefs[FIRST_DAYS])
        self.first_days_widget.setMaximum(20)
        self.first_days_widget.setToolTip('<p>Save all backup zip files for the number of days '
                                          'specified here. You can set this to zero, in which case '
                                          'only the last backup of a day will be saved</p>')
        add_right_aligned_label(l, 'Days to keep all backups:', gr)
        l.addWidget(self.first_days_widget, gr, 1)
        gr += 1

        self.more_days_widget = QSpinBox()
        self.more_days_widget.setValue(plugin_prefs[MORE_DAYS])
        self.more_days_widget.setMinimum(1)
        self.more_days_widget.setToolTip('<p>Save the last backup file made during a day for the '
        ...
        add_right_aligned_label(l, 'Days to keep last backup:', gr)
        l.addWidget(self.more_days_widget, gr, 1)
        gr += 1

        # Do this at the end so all the widgets exist
